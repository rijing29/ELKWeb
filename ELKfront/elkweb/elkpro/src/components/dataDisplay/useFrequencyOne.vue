            ...
            <el-row>
                <el-col :span="24">
                    ...
                    <el-select v-model="softwareName" placeholder="请选择软件" @change="canClick">
                        <el-option v-for="item in softwareName_options" :key="item.softwareName_value"
                        ...
                        <el-col :span="24" style="padding: 0">
                            <div class="table-wrapper">
                                ...
                                <el-table :data="tableData" style="width: 100%" :header-cell-style="{color: '#17caf0',fontSize:'16px'}" height="500">
                                    ...
                                    <el-table-column prop="COUNT" sortable label="使用次数（次）" align="center">
                                    </el-table-column>
                                </el-table>
                                <!--————表格 end————-->
                            </div>
...
export default {
    ...
    methods: {
        ...
        getModuleCount() {
            ...
            this.$http.get(url, {params}).then(res => {
                this.haveData = true
                this.tableData = res.data//表格渲染
                /*————渲染柱状图数据 begin————*/
                this.option.legend.data[0] = "软件 " + res.data[0].SOFTWARENAME
                ...
                for (i = 0; i < Math.min(res.data.length, 10); i++) {
                    this.option.yAxis[0].data[i] = res.data[i].MODULENAME
                    this.option.series[0].data[i] = res.data[i].COUNT
                    this.option.series[0].name = "软件 " + res.data[0].SOFTWARENAME
                }
                /*————渲染柱状图数据 end————*/
            })
        },
        ...
        export2Excel() {
            require.ensure([], () => {
                ...
                export_json_to_excel(tHeader, data, "软件模块使用频率excel");
            });
        },
